...
const UI = {
  ...
  showAdminLogin() {
    // Deteksi apakah sudah ada password atau masih setup awal
    const hasPassword = !!localStorage.getItem('ww_admin_hash');
    ...
    if(!hasPassword) {
      if(hint)  hint.textContent  = '✨ Pertama kali login! Buat password admin kamu.';
      if(label) label.textContent = 'Buat Password Baru (min. 4 karakter)';
      if(btn)   btn.textContent   = 'Buat Password & Masuk';
      if(input) input.placeholder = 'Ketik password baru...';
    } else {
      if(hint)  hint.textContent  = 'Masukkan password admin untuk melanjutkan.';
      if(label) label.textContent = 'Password Admin';
      if(btn)   btn.textContent   = 'Masuk';
      if(input) input.placeholder = 'Masukkan password...';
    }
...
const Admin = {
  // ─── Password System ─────────────────────────────────────────
  // Password disimpan sebagai SHA-256 hash di localStorage.
  // Pertama kali buka: tidak ada hash → mode setup (buat password baru).
  // Ganti password bisa dari tombol di Admin Panel.
  ...
  _getSavedHash() {
    try { return localStorage.getItem('ww_admin_hash') || null; }
    catch(e) { return null; }
  },

  async _saveHash(pw) {
    const h = await this._hashPw(pw);
    localStorage.setItem('ww_admin_hash', h);
    return h;
  },
  ...
  async login() {
    ...
    const savedHash = this._getSavedHash();
    const pw = document.getElementById('adminPasswordInput').value;
    if(!pw || pw.length < 1) { UI.toast('Masukkan password!', 'warn'); return; }

    // ── MODE SETUP: belum ada password tersimpan ──────────────
    if(!savedHash) {
      if(pw.length < 4) {
        UI.toast('Password minimal 4 karakter!', 'warn'); return;
      }
      await this._saveHash(pw);
      document.getElementById('adminPasswordInput').value = '';
      State.isAdmin = true;
      UI.closeAdminLogin();
      UI.showScreen('adminScreen');
      UI.toast('✅ Password admin dibuat! Selamat datang.', 'success');
      return;
    }
  ...
  // ── Ganti Password (dipanggil dari Admin Panel) ───────────────
  async changePassword() {
    ...
    if(!oldPw || !newPw || !newPw2) { UI.toast('Isi semua kolom!', 'warn'); return; }
    if(newPw !== newPw2) { UI.toast('Password baru tidak cocok!', 'warn'); return; }
    if(newPw.length < 4) { UI.toast('Password baru minimal 4 karakter!', 'warn'); return; }

    const savedHash = this._getSavedHash();
    const oldHash   = await this._hashPw(oldPw);

    if(savedHash && oldHash !== savedHash) {
      UI.toast('Password lama salah!', 'error'); return;
    }

    await this._saveHash(newPw);
  ...
  // ── Reset Password (hapus hash, kembali ke mode setup) ────────
  resetPassword() {
    if(!confirm('Reset password admin? Kamu harus buat password baru saat login berikutnya.')) return;
    localStorage.removeItem('ww_admin_hash');
    UI.toast('Password direset. Logout dan login lagi untuk buat password baru.', 'info');
  },
